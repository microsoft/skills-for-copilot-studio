    ...
    var AzureArc = class _AzureArc extends BaseManagedIdentitySource {
      ...
      async getServerTokenResponseAsync(originalResponse, networkClient, networkRequest, networkRequestOptions) {
        ...
        if (originalResponse.status === HTTP_UNAUTHORIZED) {
          ...
          let secret;
          try {
            secret = fs2.readFileSync(secretFilePath, EncodingTypes.UTF8);
          } catch (e) {
            throw createManagedIdentityError(unableToReadSecretFile);
          }
...
var LspClient = class {
  ...
  async start() {
    ...
    const pipePath = os.platform() === "win32" ? `\\\\.\\pipe\\manage-agent-${sessionId}` : path.join(os.tmpdir(), `manage-agent-${sessionId}.sock`);
...
async function cmdClone(args) {
  ...
  const clusterCategory = mgmtUrl.includes(".test.island.") ? 2 : mgmtUrl.includes(".preprod.island.") ? 3 : mgmtUrl.includes(".preview.island.") ? 4 : 5;
