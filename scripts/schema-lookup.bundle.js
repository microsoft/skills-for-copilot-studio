...
function searchDefinitions(keyword, definitions) {
  ...
  return Object.keys(definitions).filter((k) => k.toLowerCase().includes(keywordLower)).sort();
}
...
function validateYamlFile(filepath, definitions) {
  ...
  function checkVariables(obj) {
    ...
    if (typeof obj.variable === "string") {
      ...
      if (!cleanVar.startsWith("Topic.") && !cleanVar.startsWith("System.") && !cleanVar.startsWith("Global.") && !cleanVar.startsWith("User.")) {
  ...
  if (data.inputs && data.inputType) {
    ...
    const setsEqual = inputNames.size === inputTypeProps.size && [...inputNames].every((n) => inputTypeProps.has(n));
    if (setsEqual) {
